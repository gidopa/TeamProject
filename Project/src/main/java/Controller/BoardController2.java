...
@WebServlet("/board2/*")  
public class BoardController2 extends HttpServlet { //사장 
	...
	public void init(ServletConfig config) throws ServletException {
		...
		boardservice = new BoardService2();//부장 
	...
	protected void doHandle(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		switch (action) {
			...
			case "/seachlist.bo": //검색 기준값과 검색어를 입력해서 검색요청이 들어 왔을때
				...
				request.setAttribute("center", "/boarders2/list.jsp");
			...
			case "/write.bo":	//새글 입력 하는 화면 요청!
				...
				UsersVO usersvo = boardservice.serviceUsersOne(loginid);
			
				//새글 입력하는 중앙 VIEW주소 request에 바인딩
				request.setAttribute("center", "/boarders2/write.jsp");
			...
			case "/read.bo"://조회된 글목록에서 특정글의 글제목하나를 클릭했을때
				...
				int b_idx1 = Integer.parseInt(b_idx);
				list = boardservice.commentRead(b_idx1);					
				
				request.setAttribute("center", "/boarders2/read.jsp");//중앙 VIEW
				request.setAttribute("vo", vo);//응답할 Model
				request.setAttribute("list", list);//응답할 Model
			...
			case "/updateBoard.do": //글수정 요청 주소를 받았을때
				...
				case "/deleteComment.do": //댓글 삭제 요청을 받았을 때
					String _result = boardservice.serviceDeleteComment(request);
					out.write(_result);
					return;
				...
				UsersVO reply_vo = boardservice.serviceUsersOne(reply_id_);
				...
				request.setAttribute("center","/boarders2/reply.jsp");
				
				nextPage = "/project1/main.jsp";
				break;
			
			case "/replyPro.bo": // 댓글 추가 요청

			    // 답변글을 추가하기 위해 서비스 메소드 호출
			    int resultReply = boardservice.serviceReplyInsertBoard(request);
			...
			case "/site":
				request.setAttribute("center", "/list.jsp");
			...
			case "/update":
				...
				return;
			default:
				throw new IllegalArgumentException("Board .Unexpected value: " + action);
